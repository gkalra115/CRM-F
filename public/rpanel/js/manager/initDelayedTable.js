...
var KTDatatablesExtensionButtons = (function() {
  var initTable3 = function() {
    ...
    var table = $("#kt_table_3").DataTable({
      ...
      searchDelay: 500,
      ajax: {
        url: "/manager/task/delayed",
        type: "GET",
        data: {
          // parameters for custom backend script demo
          columnsDef: ["_id", "title", "softdeadline", "harddeadline", "status", "client.name"]
        }
      },
      columns: [
        { data: "_id" },
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            return `<a href="/ma/task/${data}" style="color:#000">${data}</a>`;
          }
        },
        {
          targets: 4,
...
jQuery(document).ready(function() {
  ...
  $.ajax({
    type: "GET",
    url: "/manager/task/delayed?count=yes",
  ...
  $.ajax({
    type: "GET",
    url: "/manager/task/all?count=yes",
    success: function(data) {
      $(".demo").hide("fast");
  ...
  $.ajax({
    type: "GET",
    url: "/manager/task/current?count=yes",
    success: function(data) {
      $(".demo").hide("fast");
