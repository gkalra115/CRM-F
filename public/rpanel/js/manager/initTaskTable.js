  ...
  var initTable3 = function (task, status) {
    ...
    var apiUrl =
      status !== undefined &&
        ...
        task !== '202' &&
        task !== '203'
        ? `/manager/task/filter?task=${task}&status=${status}`
        : `/manager/task/filter?task=${task}`;
    ...
    $('#kt_table_3 tbody').on('click', 'tr', function () {
      ...
      if ($('#taskId').text() !== rowData._id) {
        ...
        $.ajax({
          type: 'GET',
          url: '/manager/task/view/' + rowData._id,
          success: function (data) {
            ...
            $('#taskCreated')
              .empty()
              .append(convertTableDate(data.task[0].soft_deadline));
            $('#taskModified')
              .empty()
              .append(convertTableDate(data.task[0].hard_deadline));
              ...
              if (data.comments[j].commentFile) {
                $('#commentslist').append(
                  '<div class="kt-widget3__item"><div class="kt-widget3__header"><div class="kt-widget3__user-img"><span class="kt-badge kt-badge--username kt-badge--unified-success kt-badge--lg kt-badge--rounded kt-badge--bold" style="margin-right:10px;">' +
                    ...
                    ' </p><a taget="_blank" href="/api/solutionupload/get/taskfile/comment/' +
                    data.comments[j]._id +
                    '">' +
