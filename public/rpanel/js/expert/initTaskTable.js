  ...
  var initTable3 = function (task, status) {
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            ...
            if (full.TeamLead == undefined) {
              var lead = 'N/A';
            }
            if (full.TeamLead != undefined) {
              var lead = full.TeamLead.name;
            }
            return `
    ...
    $('#kt_table_3 tbody').on('click', 'tr', function () {
      ...
      if ($('#taskId').text() !== rowData._id) {
        ...
        $.ajax({
          type: 'GET',
          url: '/expert/task/single/' + rowData._id,
          success: function (data) {
            ...
            $('#taskdetails').attr('href', '/ex/view/' + $('#taskId').text());
            ...
            if (data.userdetails[0].Admin != undefined) {
              $('.selecttaskuser').append(
                '<option value="' +
                data.userdetails[0].Admin._id +
                '">' +
                data.userdetails[0].Admin.name +
                '</option>'
              );
            }
            if (data.userdetails[0].Manager != undefined) {
              $('.selecttaskuser').append(
                '<option value="' +
                data.userdetails[0].Manager._id +
                '">' +
                data.userdetails[0].Manager.name +
                '</option>'
              );
            }
            if (data.userdetails[0].TeamLead != undefined) {
              $('.selecttaskuser').append(
                '<option value="' +
                data.userdetails[0].TeamLead._id +
                '">' +
                data.userdetails[0].TeamLead.name +
                '</option>'
              );
            }
            if (data.userdetails[0].Expert != undefined) {
              $('.selecttaskuser').append(
                '<option value="' +
                data.userdetails[0].Expert._id +
                '">' +
                data.userdetails[0].Expert.name +
                '</option>'
              );
            }
          },
    ...
    success: function (data) {
      ...
      for (k = 0; k < data.length; k++) {
        $('#briefupload').append(
          ...
          ${data[k].fileUrl}
          " alt="" /></div><a class="kt-widget4__title" target="_blank" href="/api/solutionupload/get/taskfile/brief/${data[k]._id}" download data-toggle="kt-tooltip" data-original-title="${data[k].uploadedby.name}">${data[k].files}
    ...
    success: function (data) {
      ...
      for (var l = 0; l < data.length; l++) {
        $('#solutionupload').append(
          `<div class="kt-widget4__item"><div class="kt-widget4__pic kt-widget4__pic--icon"><img src="
          ${data[l].fileUrl}
          " alt="" /></div><a class="kt-widget4__title" target="_blank" href="/api/solutionupload/get/taskfile/solution/${data[l]._id}" download data-toggle="kt-tooltip" data-original-title="${data[l].uploadedby.name
          }">${data[l].files}
            </a><span style="text-align:right;">${timesend(
            data[l].createdAt
          )}</span></div>`
  ...
  swal
    ...
    .then(function (result) {
      if (result.value) {
        $.ajax({
          method: 'PUT',
          url: '/expert/task/status/' + $('#taskAssignId').text(),
          success: function (data) {
            if ($.fn.DataTable.isDataTable('#kt_table_3')) {
              $('#kt_table_3').DataTable().destroy();
            }
