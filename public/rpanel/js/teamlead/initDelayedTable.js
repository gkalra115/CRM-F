...
var KTDatatablesExtensionButtons = (function() {
  var initTable3 = function() {
    ...
    var table = $("#kt_table_3").DataTable({
      ...
      searchDelay: 500,
      ajax: {
        url: "/teamlead/task/delayed",
        type: "GET",
        data: {
          // parameters for custom backend script demo
          columnsDef: ["_id", "title", "deadline", "status", "manager.name", "expert.name"]
        }
      },
      columns: [
        { data: "_id" },
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            return `<a href="/te/view/${data}" style="color:#000">${data}</a>`;
          }
        },
        {
          targets: 3,
        ...
        },
        {
          ...
          render: function(data, type, full, meta){
            if(data == undefined){
              ...
              return (
               ` <span style="width: 110px;"><span class="kt-font-bold kt-font-warning">${data}</span></span>`
              );
            }
          }
        },
        {
          ...
          render: function(data, type, full, meta){
            if(data == undefined){
              ...
              return (
                ` <span style="width: 110px;"><span class="kt-font-bold kt-font-brand">${data}</span></span>`
               );
            }
          }
        }
...
jQuery(document).ready(function() {
  ...
  $.ajax({
    type: "GET",
    url: "/teamlead/task/delayed?count=yes",
  ...
  $.ajax({
    type: "GET",
    url: "/teamlead/task/all?count=yes",
    success: function(data) {
      $(".demo").hide("fast");
  ...
  $.ajax({
    type: "GET",
    url: "/teamlead/task/current?count=yes",
    success: function(data) {
      $(".demo").hide("fast");
