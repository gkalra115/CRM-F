...
var KTDatatablesExtensionButtons = (function () {
  var initTable3 = function () {
    ...
    var table = $("#kt_table_3").DataTable({
      ...
      searchDelay: 500,
      ajax: {
        url: `/teamlead/task/effort?month=${date.getUTCMonth() + 1}&year=${date.getUTCFullYear()}`,
        type: "GET",
        data: {
    ...
    $('#kt_dashboard_daterangepicker_3').on(
      ...
      (ev, picker) => {
        ...
        table.ajax
          .url(`/teamlead/task/effort?month=${date[0]}&year=${date[1]}`)
          .load();
      }
    );
    ...
    $('#tillEot').on('click', () => {
      ...
      table.ajax.url(`/teamlead/task/effort`).load();
    });
  };
  return {
...
var clientfinancestats = function (value) {
  ...
  if ((cMonth === 'EOT', gMonth === 'EOT', cYear === 'EOT')) {
    apiUrl = `/teamlead/task/effort`;
    $(".kt-widget__info").hide()
    $("#ttotal").hide()
  } else {
    apiUrl = `/teamlead/task/effort?month=${cMonth}&year=${cYear}`;
...
function validateTask() {
  ...
  if (taskid == "") {
    ...
    $.ajax({
      type: 'GET',
      url: '/teamlead/task/validate/' + taskid,
      success: function (data) {      
          $('#taskkeyerror').empty();
