...
$.ajax({
  ...
  success: function (data) {
    clients = `<option value="${data._id}">${data.name}</option>`;
    $('#whatsapp').on('click', function () {
      let taskurl = 'https://api.whatsapp.com/send?phone=' + data.phone;
      window.open(taskurl, '_blank');
    });
    $('#clientinitials').empty().append(data.name.charAt(0));
    $('#clientname').empty().append(data.name);
    $('#clientemail')
      .empty()
    ...
    $('#clientbdm')
      ...
      .html(
        "<i class ='flaticon2-user'></i>" +
          (data.assignedTo ? data.assignedTo.name : 'N/A')
      );
