...
var KTDatatablesExtensionButtons = (function () {
  ...
  var initTable3 = function () {
    ...
    var table = $('#kt_table_3').DataTable({
      ...
      searchDelay: 500,
      ajax: {
        url: `/api/task/payment/tasklist?month=${currentMonth}&year=${currentYear}`,
        type: 'GET',
        data: {
          columnsDef: ['_id', 'title', 'wordcount','hard_deadline','client.name', 'BDA.name', 'amount'],
        },
      },
      columns: [
        { data: '_id' },
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            ...
            if(data){
              ...
              var centpaid = parseFloat(((amount - paid)/amount)).toFixed(2);
              pstatus = String((centpaid == 0.00 ? 'Paid' : (centpaid == 1.00 ? 'Unpaid':'Partial')))
            } 
            else {
              pstatus = String('N/A')
            }
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            return `<a href="/su/task/${data}" target="_blank" style="color:#000; text-decoration:none;">${data}</a>`;
          },
        },
        {
          targets: 3,
    ...
    $('#paid_status').on('click', function (e) {
      e.preventDefault();
      table.columns(6).search("^" +'Paid'+ "$", true, false, true).draw();
    });
    $('#unpaid_status').on('click', function (e) {
      e.preventDefault();
      table.columns(6).search("^" +'Unpaid'+ "$", true, false, true).draw();
    });
    $('#partial_status').on('click', function (e) {
      e.preventDefault();
      table.columns(6).search('Partial').draw();
    });
    $('#na_status').on('click', function (e) {
      e.preventDefault();
      table.columns(6).search('N/A').draw();
    });
    ...
    $('#kt_dashboard_daterangepicker_3').on(
      ...
      (ev, picker) => {
        ...
        table.ajax
          .url(`/api/task/payment/tasklist?month=${date[0]}&year=${date[1]}`)
          .load();
      }
    );
    ...
    $('#tillEot').on('click', () => {
      ...
      table.ajax.url(`/api/task/payment/tasklist`).load();
    });
